...
public class ExternalURIElementBuilder implements MetaBuilder<ExternalURIElement> {

    private DiscriminatorMap discriminatorMap;

    public ExternalURIElementBuilder(DiscriminatorMap discriminatorMap) {
        this.discriminatorMap = discriminatorMap;
    }

    public ExternalURIElementBuilder() {
        this(null);
    }

    @Override
    public ExternalURIElement build(String tag, MetaInputStream inputStream) throws IOException, UnknownDiscriminatorException {
        try {
            return new ExternalURIElement(new URI(inputStream.readUTF()),
    ...
    @Override
    public ExternalURIElement build(String tag, MetaByteBuffer buffer) throws UnknownDiscriminatorException {
        try {
            return new ExternalURIElement(new URI(buffer.readUTF()),
    ...
    @Override
    public Class<ExternalURIElement> getDataClass() {
        return ExternalURIElement.class;
    }
